...
# код типа без дополнительных заморочек, то используем самописный
# дескриптор. Он будет хранить функции получения и записи значения в
# массив для нужных типов. Упрощенный аналог дескриптора из модуля array:
...
cdef int double_setitem(array a, size_t index, object obj):
    ...
    # самими извлечь значение из объекта PyObject.
...
cdef int int_setitem(array a, size_t index, object obj):
    ...
    if not isinstance(obj, int) and not isinstance(obj, float):
        return -1
    # Преобразования Python объекта в Сишный
    cdef int value = PyFloat_AsDouble(obj)
    if index >= 0:
        # Не забываем преобразовывать тип, т.к. a.data имеет тип char
...
cdef class array:
    ...
    def initialize(self):
        # Объявление переменно цикла позволяет эффективнее компилировать код.
        cdef int i
    ...
    def append(self, object value):
        id = self.length + 1
        self.data = <char *> PyMem_Realloc(self.data, id * self.descr.itemsize)
        self.__setitem__(id, value)

    def insert(self, size_t index, object value):
        length = self.length + 1
        self.data = <char *> PyMem_Realloc(self.data, length * self.descr.itemsize)
        for i in range(length - 1, index, -1):
            new_value = self.__getitem__(i - 1)
            self.__setitem__(i, new_value)
